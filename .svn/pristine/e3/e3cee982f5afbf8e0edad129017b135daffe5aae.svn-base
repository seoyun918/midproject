...
public class insertVideo extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		for(Part part : request.getParts()) {
			...
			if(!"".equals(fileName)) {
				String uploadPath = "D:/A_TeachingMaterial/4.MiddleProject/workspace/soju/WebContent/video/" + bd_num;
		...
		response.sendRedirect(request.getContextPath() + "/jsp/media/videoDetail.jsp");
	}
